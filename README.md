...
Cette simplification ne pose pas de problème. En effet, comme le RightVector est certainement à droite, et que le LookVector et UpVector sont déjà défini, on ne peut que se retrouver avec des *carts* orientés dans la bonne direction. On peut régler toutefois toute forme de doute en dissociant le *cart* qui est effectivement sur la *section* et utilise la CFrame calculer, et le modèle apparent de ce *cart*, qui sera sûrement géré localement, par le client.
